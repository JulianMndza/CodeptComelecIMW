                    ...
                    <%
                        try {
                            if (conn != null) {
                                ...
                                String query = "SELECT * FROM SCHOOL_YEARS";
                                ResultSet rs = st.executeQuery(query);
                                int counter = 1;
                                rs.afterLast();
                                while (rs.previous()) {
                                    schoolYear = rs.getString("SCHOOL_YEAR").trim();
                    %>
                    <form action="Units_Archive" method="get" class="year-wrapper">
                        <input type="hidden" name="archivechosenunit" value="<%out.print(chosenUnit);%>">
                        <input type="hidden" name="schoolyear" value="<%out.print(schoolYear);%>">
                        <button type="submit" class="year-button">  
                            <h3><%out.print(schoolYear);%></h3>
                        </button>  
                    </form>
                    <%
                                    counter++;
